...
Trained on CPU only in ~45 minutes. Competitive with published GNN baselines on QM9.
...
```
Raw molecule (SMILES / 3D geometry)
        │
        ▼
...
This project uses the first **20,000 molecules** for computational efficiency.
...
![Attention Heatmaps](attention_heatmaps.png)
...
- Loss converges smoothly; the LR scheduler reduces the learning rate 4 times across 100 epochs.
- No early stopping was triggered — the model continued improving until epoch 77 (best val MAE).
...
```bash
git clone https://github.com/YOUR_USERNAME/molecular-graph-transformer.git
...
The notebook downloads QM9 automatically on first run (~80 MB).
...
| CPU only, ~45 min training | GPU training would allow larger hidden dim / more layers |
...
| No 3D geometry beyond bond length | Full 3D architecture (DimeNet++, SphereNet) |
